...
module NHSChoicesAPI
  ...
  NHSAPIKEY = 'NOCRIYLM'
  API_URL = 'http://v1.syndication.nhschoices.nhs.uk/organisations/treasures/'
  
  class Scraper
    # Get the available overview details for a given location ID
    def get_treasure_overview(id = nil)
      ...
      url = make_treasure_overview_url id
    ...
    def get_treasure_overview_urls
      @overview_urls ||= get_treasure_overview_urls_from_server
    end

    # Get an array of treasure ids. Memoizes the result to @treasure_ids
    def get_treasure_ids
      @treasure_ids ||= get_treasure_ids_from_server
    end

    # All methods past this point will not be publicly available
    private

    def make_treasure_overview_url(id = nil)
      return nil if id.equal? nil
    ...
    def get_treasure_overview_urls_from_server
      overview_urls = {}
      ids = get_treasure_ids
    ...
    # Scrapes the NHS Choices API for treasure ids
    def get_treasure_ids_from_server
